...
public class haikin_third extends AppCompatActivity {
    ScrollView scrollView;
    TestOpenHelperForTitle helper;
    SQLiteDatabase db;
    String today_bird_dog_title = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.haikin_third);
        ...
        //筋トレ方法名と日付を連結
        today_bird_dog_title = date + "_bird_dog";


        //日付を含む筋トレ方法についてのデータをデータベースに登録(重複無し)
        ContentValues val = new ContentValues();
        val.put("title", today_bird_dog_title);
        val.put("score", 0);
        db.insert("title_table", null, val);



        View bird_dog_movie = findViewById(R.id.bird_dog_movie);
        bird_dog_movie.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                int bird_dog_check = bird_dog_readData();  //現在の動画再生数を確認

                if(bird_dog_check ==0) {
                    bird_dog_check++;
                }else if(bird_dog_check == 1){
                    bird_dog_check++;
                }else if(bird_dog_check == 2){
                    bird_dog_check++;
                }

                bird_dog_upDate(db,bird_dog_check); //再生数を更新(最大で3まで)

                Intent intent = new Intent(getApplication(),movie_play_scene.class);
                // 移行後に再生する動画を指定
                intent.putExtra("mp4", R.raw.bird_dok);
                // 画面移行スタート
                startActivity(intent);
            }
        });
        ...
        Button bird_dog_CountButtonForMinus = findViewById(R.id.bird_dog_minus);
        bird_dog_CountButtonForMinus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                TextView text = findViewById(R.id.bird_dog_countNum);
                int num = Integer.parseInt(text.getText().toString());
                if(num > 0) {
                    num--;
                }
                today_bird_dog_upDate(db,num);
        ...
        Button bird_dog_CountButtonForPlus = findViewById(R.id.bird_dog_plus);
        bird_dog_CountButtonForPlus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                TextView text = findViewById(R.id.bird_dog_countNum);
                int num = Integer.parseInt(text.getText().toString());
    ...
    public void onResume(){
        ...
        ImageView bird_dog_star1 = findViewById(R.id.bird_dog_star1);
        ImageView bird_dog_star2 = findViewById(R.id.bird_dog_star2);
        ImageView bird_dog_star3 = findViewById(R.id.bird_dog_star3);


        int bird_dog_score = bird_dog_readData();


        if(bird_dog_score >= 1){
            bird_dog_star1.setImageResource(R.drawable.cleared_star);
        }
        if(bird_dog_score >= 2){
            bird_dog_star2.setImageResource(R.drawable.cleared_star);
        }
        if(bird_dog_score >= 3){
            bird_dog_star3.setImageResource(R.drawable.cleared_star);
        }



        //日付を含む筋トレ方法のデータを読み込んで回数を表示
        String today_bird_dog_num = Integer.toString(today_bird_dog_readData());
        TextView text = findViewById(R.id.bird_dog_countNum);
        text.setText(today_bird_dog_num);

    }

    private int bird_dog_readData() {
        if (helper == null) {
            helper = new TestOpenHelperForTitle(getApplicationContext());
        }
        ...
        Cursor cursor = db.rawQuery("select title ,score from title_table where title = 'bird_dog_check'", null);
    ...
    public void bird_dog_upDate(SQLiteDatabase db,int score){
        if(helper == null){
            helper = new TestOpenHelperForTitle(getApplicationContext());
        }
        ...
        db.update("title_table",values,"title=?",new String[]{"bird_dog_check"});
    }


    public void today_bird_dog_upDate(SQLiteDatabase db,int score){
        if(helper == null){
            helper = new TestOpenHelperForTitle(getApplicationContext());
        }
        ContentValues values = new ContentValues();
        values.put("score", score);
        db.update("title_table",values,"title=?",new String[]{today_bird_dog_title});
    }


    private int today_bird_dog_readData() {
        if(helper == null){
            helper = new TestOpenHelperForTitle(getApplicationContext());
        }
        SQLiteDatabase db = helper.getReadableDatabase();
        ...
        //カーソルを移動して上から順に読み込んでるイメージ。
        Cursor cursor = db.rawQuery("select title ,score from title_table where title = '" + today_bird_dog_title + "'", null);
        boolean next = cursor.moveToFirst();
        while (next) {
            // 取得したカラムの順番(0から始まる)と型を指定してデータを取得する
            scoreData = cursor.getInt(1);// scoreを取得
